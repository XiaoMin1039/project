    ...
    select.prototype.init = function () {
        ...
        _self.$ele.on('click','span', function (e) {
            ...
            if ($(this).attr('data-type') === 'unit') {
                ...
                $(this).find('input').bind('blur',function (e) {
                    e.stopPropagation();
                    var name = $(this).parents('span').attr('data-value');
                    var unit = _self.getUnit(name);
                    var val = $(this).val();
                    if(unit.multiple){
                        $(this).val(parseInt(parseInt(val/unit.multiple)*unit.multiple))
                    }
                })
            } else {
    ...
    select.prototype.append = function (option) {
        if(option.type == 'hRent'){
            ...
            if (option.type === 'unit') {
                ...
                if(option.unit.content) pHtml='<p>'+option.unit.content+'</p>';
                var top=this.$ele.children('span:last').height()+10+'px';
                var arrowLeft=this.$ele.children('span:last').width();
                this.$ele
                    .children('span:last')
                    .append(`
