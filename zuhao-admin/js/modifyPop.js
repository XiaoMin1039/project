 ...
 */
(function($){
    function modifyPop(ele,options){
        this.$ele=ele;
    ...
    modifyPop.prototype.init=function(){
        var _self=this;
        ...
        var html=$(`
              <div class="event-popup absolute opacity0 minPop" data-garnish="event-popup-top"  data-id="${_index}">
                ...
                <input type="text" placeholder="${_self.options.inputTip || ''}">
        ...
        if($('body').find('[data-id="'+_index+'"]').length === 0){
            ...
            _con.find('.button-event-small-blue').click(function(){
                ...
                _self.hidePop(_index);
                _self.options.confirm(_con.find('input').val());
            });
            // 点击其他地方，窗口消失
    ...
    modifyPop.prototype.getPosition=function(ele){
        /*获取元素的纵坐标*/
        function getTop(e){
    ...
    modifyPop.prototype.locationPop=function(_con,pos){
    ...
    modifyPop.prototype.hidePop=function(id){
        //动画消失
        theFading($('body').find('[data-id="'+id+'"]')[0],0,function(){
            $('body').find('[data-id="'+id+'"]').remove();
        });
    };
    $.fn.modifyPop=function(options){
        return new modifyPop(this,options)
    }
})(jQuery);