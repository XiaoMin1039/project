  ...
  <div class="withdraw-cash">
    ...
    <div class="result" v-show="!(loading || unfind)">
      <el-table :data="list" style="width: 100%" :row-class-name="tableRowClassName">
        ...
        <el-table-column label="用户账号" width="160">
          <template scope="scope">
    ...
    <el-dialog class="dialog" :title="`处理用户：${info.userAccount} 的提现`" :visible.sync="dialog" size="tiny" :before-close="dialogClose">
      <p>提现金额：
...
export default {
  ...
  methods: {
    submitResult() {
      ...
      this.axios.post('/admin/withdraw/user/result', data).then(({ data }) => {
    ...
    uncheck(row) {
      this.axios.post('/admin/withdraw/user/cancle_check', { recharge_id: row.rechargeId, admin_id: 1 }).then(({ data }) => {
        if (data.content !== 1) this.$message.error('服务器发生错误')
        else {
          this.list = this.list.map(item => {
            let t = item
    ...
    dialogClose(done) {
      this.axios.post('/admin/withdraw/user/cancle_check', { recharge_id: this.info.rechargeId, admin_id: 1 }).then(({ data }) => {
        if (data.content !== 1) this.$message.error('服务器发生错误')
        else {
          this.list = this.list.map(item => {
            let t = item
    ...
    check(row) {
      this.info = row
      this.axios.post('/admin/withdraw/user/check', { recharge_id: row.rechargeId }).then(({ data, status }) => {
        if (data.content != 1) {
          ...
          this.info = data.userRechargeDto
          this.dialog = true
        }
      })
    },
    ...
    fetchData(page, first) {
      ...
      this.axios.post('/admin/withdraw/user/gets', data).then(({ data, status }) => {
        this.loading = false
        if (data.content !== 1) {
          this.$message.error('服务器错误！')
        } else {
          this.list = data.userRecharges
          if (first) this.totPage = data.countPage
          if (data.userRecharges.length == 0) this.unfind = true
        }
      }).catch(err => {
        this.$message.error('服务器错误！')
      })
